...
+++Het equivalent Web SDK van het platform dat het `applyPropositions` bevel gebruikt: 
...
Het Web SDK van het Platform biedt grotere controle voor het toepassen van op vorm-gebaseerde activiteiten op de pagina gebruikend het `applyPropositions` bevel met `actionType` gespecificeerd:
...
+++Platform Web SDK voorbeeld met veelvoudige werkingsgebieden
