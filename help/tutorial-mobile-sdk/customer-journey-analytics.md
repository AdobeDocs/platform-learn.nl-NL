...
De mobiele gegevens van de toepassingsgebeurtenis, die u verzamelde en naar de Edge Network van het Platform in vroegere lessen verzond wordt door:sturen aan de diensten die in uw gegevensstroom worden gevormd. Als u de [Gegevens naar Experience Platform verzenden](platform.md) les, dat de gegevens nu in het gegevensmeer van Experience Platform als datasets worden opgeslagen. Die gegevens zijn vervolgens beschikbaar voor Customer Journey Analytics om te gebruiken voor rapportage en analyse.

In tegenstelling tot Adobe Analytics is Customer Journey Analytics een toepassing die *gebruik* gegevens uit gegevenssets die u in Experience Platform hebt gemaakt en waarnaar uw app gegevens verzendt. Met de Adobe Experience Platform Mobile SDK verzendt u geen gegevens rechtstreeks naar de Customer Journey Analytics. In plaats daarvan, gebruikt de Customer Journey Analytics de gegevens van de datasets in Experience Platform.

Deze les in de zelfstudie is gericht op het rapporteren en analyseren van de gegevens die zijn vastgelegd in de zelfstudie-app Luma. Één van de unieke mogelijkheden van Customer Journey Analytics is gegevens van veelvoudige bronnen (CRM, verkooppunt, loyaliteitstoepassing, call-center) en kanalen (Web, mobiel, off-line) te combineren om diepgaande inzichten in klantenreizen te verstrekken. Dat vermogen is voorbij het werkingsgebied van deze les. Zie [Overzicht van Customer Journey Analytics](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-overview/cja-overview) voor meer informatie .


## Vereisten

Uw organisatie moet provisioned en toestemming voor Customer Journey Analytics worden verleend. U moet beheertoegang tot Customer Journey Analytics hebben.
...
Deze bestelling is bedoeld. In Customer Journey Analytics, hangt een rapport in Analysis Workspace van een gegevensmening af. En een gegevensmening hangt van een verbinding af.
...
1. In de **[!UICONTROL Components]** tabblad van het **[!UICONTROL Luma App - AEP Mobile SDK Tutorial Data view]** kunt u de metriek en de dimensie definiëren die u wilt gebruiken wanneer u rapporteert over uw mobiele app. Door gebrek, worden een aantal standaardmetriek en afmetingen (gezamenlijk die naar een componenten worden verwezen) reeds gevormd voor uw gegevensmening. Maar uw gegevensweergave vereist meer componenten. <br/>Om een schemagebied, van uw eerder bepaald schema of uit de vakschema&#39;s toe te voegen (zie [Een schema maken](create-schema.md) les), als component (afmetingen of metrisch):
   ...
   1. Nu u begrijpt hoe u velden aan uw gegevensweergave kunt toevoegen en de resulterende component kunt configureren, gebruikt u de onderstaande tabellen voor een lijst met schemavelden die u als metrisch of dimensionaal wilt toevoegen. Gebruik de **Schemapad** de kolomwaarde van de lijst hieronder om naar het specifieke schemagebied te zoeken of te doorkruisen. Nadat u het gereedschap hebt gesleept en neergezet, controleert u de knop **Componentinstellingen** kolomwaarde in de tabel of specifieke instellingen zijn vereist voor een component, zoals het wijzigen van de kolomwaarde **[!UICONTROL Component name]** of definiëren **[!UICONTROL INCLUDE EXCLUDE VALUES]**.
      ...
      **METRISCHE**
      ...
      Nadat u alle schemagebieden van de lijst hierboven als metrische component hebt toegevoegd, uw configuratie van de gegevensmening voor **[!UICONTROL METRICS]** zou als moeten kijken:
      ...
      **DIMENSIONEN**
      ...
      Nadat u alle schemagebieden van de lijst hierboven als afmetingscomponent hebt toegevoegd, uw configuratie van de gegevensmening voor **[!UICONTROL DIMENSIONS]** zou als moeten kijken:
...
U gebruikt de projecten van de Werkruimte in Customer Journey Analytics om uw rapporten en visualisaties te bouwen. Er zijn vele mogelijkheden om uitvoerige rapporten en het in dienst nemen van visualisaties te bouwen, maar al deze mogelijkheden zijn buiten het werkingsgebied van deze zelfstudie. Zie [Overzicht van werkruimte](https://experienceleague.adobe.com/en/docs/customer-journey-analytics-learn/tutorials/analysis-workspace/workspace-projects/analysis-workspace-overview) en [Een nieuw project maken](https://experienceleague.adobe.com/en/docs/customer-journey-analytics-learn/tutorials/analysis-workspace/workspace-projects/build-a-new-project) voor meer informatie .
...
U hebt nu uw project ingesteld. U hebt al een deelvenster Vrije vorm op het hoofdcanvas met een tabel voor vrije vorm. U gaat binnenkort componenten aan deze tabel toevoegen, maar u moet er eerst voor zorgen dat het deelvenster Vrije vorm de juiste gegevensweergave en de juiste tijdsperiode gebruikt.
...
1. Als u de juiste tijdsperiode voor het deelvenster wilt definiëren, selecteert u de standaardinstelling **[!UICONTROL This month]** en definieert u in het pop-upvenster de begin- en einddatum. Of gebruik een **[!UICONTROL Preset]**, zoals **[!UICONTROL Last 6 full months]** en selecteert u **[!UICONTROL Apply]**.
   ![CJA-project 6](assets/cja-projects-6.png)


### Toepassingsgebruik

U wilt rapporteren hoe de app wordt gebruikt. U hebt de vereiste code toegevoegd aan de app om de interacties tussen apps te registreren en welke schermen in de app worden gebruikt (zie de [Gebeurtenissen bijhouden](events.md) les) en u wilt nu over deze gegevens rapporteren.

#### Schermnamen

U wilt eerst rapporteren over welke schermen in de app zijn weergegeven.
...
Uw vrije lijst toont nu het schermmeningen voor de dagen voor uw geselecteerde tijdspanne. U wilt echter wel schermweergaven weergeven voor de verschillende schermen die in de app worden gebruikt.
...
Uw eerste rapport is gereed. Schermweergaven worden getoond voor de verschillende schermnamen die u in de app hebt gedefinieerd.
...
U wilt ook rapporteren over de manier waarop gebruikers met de app hebben gewerkt.
...
Uw **[!UICONTROL Commerce Events]** het rapport is nu klaar en toont hoe gebruikers producten bekeken , producten toegevoegd aan hun verlanglijst , producten opgeslagen voor later of zelfs aangeschaft hebben .
...
Gebaseerd op het vorige rapport, wilt u de reserve in de handelskrechter visualiseren: hoeveel gebruikers die producten bekeken ook toevoegden producten aan karretjes. En hoeveel gebruikers die producten aan hun winkelwagentjes hebben toegevoegd, hebben deze producten ook voor later opgeslagen. En zo verder.
...
U hebt nu een rapport met de maandelijkse aanbiedingen die worden weergegeven aan uw gebruikers in de app.
...
Er wordt nu een rapport weergegeven met de afzonderlijke voorstellen die voor de geselecteerde periode worden weergegeven voor de beslissing over deze aanbieding voor gebruikers van uw app.
...
U hebt nu een rapport waarin gebruikers worden weergegeven die zich in en buiten de nabijheid van uw opslaglocaties bevinden (aangezien u deze locaties hebt gedefinieerd in het dialoogvenster [Plaatsen](places.md) les).
