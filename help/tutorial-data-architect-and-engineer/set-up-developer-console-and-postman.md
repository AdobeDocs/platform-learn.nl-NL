...
Adobe Developer Console is de ontwikkelaarsbestemming om toegang te krijgen tot Adobe API&#39;s en SDK&#39;s, te luisteren naar gebeurtenissen in real time, functies uit te voeren bij uitvoering of insteekmodules of App Builder-toepassingen te bouwen. U gebruikt deze voor toegang tot de Experience Platform API. Voor meer details, zie de [&#x200B; documentatie van Adobe Developer Console &#x200B;](https://www.adobe.io/apis/experienceplatform/console/docs.html)
...
1. Selecteer **[!UICONTROL Try it out]** in de sectie **[!UICONTROL Download for Postman]** van de pagina en selecteer **[!UICONTROL OAuth Server-to-Server]** om het [!DNL Postman] omgeving-json-bestand te downloaden. Sla `oauth_server_to_server.postman_environment.json` op in de map `Luma Tutorial Assets` .
...
Als u de API-referentie wilt gebruiken om te communiceren met Experience Platform, moet u een System Admin (Systeembeheerder) de API-referenties toewijzen aan de rol die in de vorige les is gemaakt.  Als u geen Systeembeheerder bent, verzendt u deze:
...
1. De schakelaar aan uw Webbrowser en kijkt omhoog huurder identiteitskaart van uw bedrijf door naar de interface van Experience Platform te gaan en het gedeelte van URL *na @ teken* te halen. Mijn huurder-id is bijvoorbeeld `techmarketingdemos` , maar die van u is anders:
...
1. Voer `global` in beide waardevelden in
   ...
   >[!NOTE]
   >
   >`CONTAINER_ID` is een veld waarvan de waarde tijdens de zelfstudie meerdere keren wordt gewijzigd. Wanneer `global` wordt gebruikt, communiceert de API met door Adobe verschafte elementen in uw Platform-account. Wanneer `tenant` wordt gebruikt, communiceert de API met uw eigen aangepaste elementen.
...
Adobe biedt een uitgebreide set [!DNL Postman] -verzamelingen waarmee u de Experience Platform API kunt verkennen. Deze inzamelingen zijn in de [&#x200B; Postman Steekproeven GitHub repo van Adobe Experience Platform &#x200B;](https://github.com/adobe/experience-platform-postman-samples). U zou referentie dit repo aangezien u dit vele tijden door dit leerprogramma en later zult gebruiken aangezien u Experience Platform voor uw eigen bedrijf implementeert.
...
Open de [&#x200B; inzamelingen van 0&rbrace; Experience Platform &lbrace;in GitHub  [!DNL Postman] . &#x200B;](https://github.com/adobe/experience-platform-postman-samples/tree/master/apis/experience-platform) Deze pagina bevat veel verzamelingen voor verschillende platform-API&#39;s. Ik beveel ten stelligste aan dat er een bladwijzer wordt gemaakt.
