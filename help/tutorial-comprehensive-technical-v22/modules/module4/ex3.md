---
title: Query-service - query's, query's, query's... en kurkanalyse
description: Query-service - query's, query's, query's... en kurkanalyse
kt: 5342
...
# 4.3 Vragen, query&#39;s, query&#39;s... en kurkanalyse
...
Alle vragen die in dit hoofdstuk worden vermeld, worden uitgevoerd in uw **PSQL-opdrachtregelinterface**. Kopieer (CTRL-c) de instructieblokken die met **SQL** en plak (CTRL-v) ze in de **PSQL-opdrachtregelinterface**. De **Zoekresultaat** de blokken tonen de geplakte SQL verklaring en het bijbehorende vraagresultaat.

## 4.3.1 Basisvragen voor gegevensanalyse schrijven

### Tijdstempel

Gegevens die in Adobe Experience Platform zijn vastgelegd, krijgen een tijdstempel. De **tijdstempel** kunt u gegevens in de loop van de tijd analyseren.
...
Kopieer de bovenstaande instructie en voer deze uit in uw **PSQL-opdrachtregelinterface**.
...
Adobe heeft een set toegevoegd **Door Adobe gedefinieerde functies** in de standaard SQL syntaxis die u toestaat om uw ervaringsgegevens beter te begrijpen. In de volgende paar vragen zult u over deze functies ADF leren. U vindt meer informatie en de volledige lijst [in de documentatie](https://experienceleague.adobe.com/docs/experience-platform/query/sql/adobe-defined-functions.html).

### Wat doen mensen op de site voordat ze de pagina &quot;Service annuleren&quot; bereiken als derde pagina in een sessie?

Met deze vraag zult u de eerste twee Adobe bepaalde Functies ontdekken **SESS_TIMEOUT** en **VOLGENDE**

> De **SESS_TIMEOUT()** geeft een overzicht van de met Adobe Analytics gevonden bezoeken. Het voert een gelijkaardige op tijd-gebaseerde groepering uit, maar klantgerichte parameters.
>
> **NEXT()** en **VORIGE()** helpen u te begrijpen hoe klanten door uw site navigeren.
...
Om dit soort vraag te beantwoorden zullen wij gebruiken **TIME_BETWEEN_NEXT_MATCH()** Adobe gedefinieerde functie.
...
Verklaar dat wij samen datasets samenvoegen, in dit geval sluiten wij ons aan bij ons `demo_system_event_dataset_for_website_global_v1_1` with `demo_system_event_dataset_for_call_center_global_v1_1`. We doen dit om het resultaat van de interactie van het callcenter te kennen.
...
In deze query voegen we ons aan bij de loyaliteitsgegevens die we in Adobe Experience Platform hebben ingezien. Dit staat toe om de kinneanalyse met loyaliteitsgegevens te verrijken.
...
Klik in Windows op het pictogram &quot;Zoeken&quot; (1) op de werkbalk Vensters en typ **laptop** Klik in het veld &quot;search&quot;-field (2) op (3) het &quot;notebookresultaat&quot;:
...
Niet verwijderen `/\*` en `\*/`. De aangepaste instructie in de laptop moet er als volgt uitzien:

![edit-query-notepad.png](./images/edit-query-notepad.png)

De gewijzigde instructie kopiëren uit **laptop** in de **PSQL-opdrachtregelvenster** en druk op Enter. U zou het volgende resultaat in het de bevellijnvenster van PSQL moeten zien:
...
In volgende u zult uw vraag voortzetten (die ook als wordt bekend **tabel maken zoals u selecteert** of **CTAS**) als een nieuwe dataset die u in de Power BI van Microsoft zult gebruiken.

Volgende stap: [4.4 Genereer een dataset van een vraag](./ex4.md)
